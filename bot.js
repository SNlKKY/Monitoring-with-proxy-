...
async function checkOnceInternal(username, useProxy) {
  ...
  try {
    ...
    // Track bandwidth used for this single check (sum of response content-lengths).
    let bytesUsed = 0;
    page.on('response', (res) => {
      const len = res.headers()['content-length'];
      if (len) bytesUsed += parseInt(len, 10);
    });
    ...
    console.log(`[check] ${username} og:title:`, (data.title || '(empty)').substring(0, 100));
    console.log(`[check] ${username} pageText:`, (data.pageText || '(empty)').substring(0, 150));
    ...
    const totalKB = (bytesUsed / 1024).toFixed(1);
    console.log(`[check] ${username} bandwidth used: ${totalKB} KB (page only, excludes profile pic logged above)`);
    ...
    // Fallback heuristic: a normal profile's og:title looks like
    // "Name (@username) • Instagram photos and videos". If og:title is empty,
    // or is just the generic "Instagram" with no username in it, that's a strong
    // sign the profile no longer exists — even if the exact wording above didn't match.
    ...
    if (!followers && !following) {
      if (explicitlyUnavailable || titleLooksGeneric) {
        // Real signal that the account is gone/banned/deleted.
    ...
    console.log(`[check] ${username} isVerified: ${data.isVerified}`);
    return { banned: false, followers, following, posts, profilePic, bio: '', isVerified: data.isVerified };
