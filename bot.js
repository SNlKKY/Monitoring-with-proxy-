...
function registerCardFonts() {
  ...
  // Candidate paths: Debian/Ubuntu liberation fonts (installed via apt "fonts-liberation"),
  // plus an optional bundled ./fonts folder as a fallback for other environments (e.g. Windows exe).
  const candidates = [
    { path: '/usr/share/fonts/truetype/liberation/LiberationSans-Regular.ttf', family: 'CardFont' },
    { path: '/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf',    family: 'CardFont-Bold' },
    { path: path.join(global.__basedir || __dirname, 'fonts', 'LiberationSans-Regular.ttf'), family: 'CardFont' },
    { path: path.join(global.__basedir || __dirname, 'fonts', 'LiberationSans-Bold.ttf'),    family: 'CardFont-Bold' },
  ];
  ...
  for (const c of candidates) {
    try {
      if (fs.existsSync(c.path)) {
        ...
        console.log(`[font] registered ${c.family} from ${c.path}`);
      }
    } catch (e) {
      console.log(`[font] failed to register ${c.path}:`, e.message);
    }
  }
  ...
  try {
    ...
    if (profilePicUrl) {
      try {
        ...
        ctx.fillStyle = '#888';
        ctx.font = `bold ${AVATAR_SIZE * 0.4}px CardFont-Bold`;
    ...
    ctx.fill();
    ctx.fillStyle = '#fff';
    ctx.font      = 'bold 13px CardFont-Bold';
    ...
    ctx.fill();
    ctx.fillStyle = '#fff';
    ctx.font      = 'bold 15px CardFont-Bold';
    ...
    for (const s of stats) {
      // Bold number
      ctx.fillStyle = '#ffffff';
      ctx.font      = 'bold 16px CardFont-Bold';
      ctx.fillText(s.num, sx, ry);
      // Small grey label below
      ctx.fillStyle = '#aaaaaa';
      ctx.font      = '12px CardFont';
